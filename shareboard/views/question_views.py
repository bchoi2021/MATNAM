...
from shareboard.forms import BoardWriteForm, AnswerForm
from shareboard.models import ShareBoard
...
def index(request):
    ...
    return render(request, 'shareboard/board_list.html', context)
...
def board_list(request):    
    ...
    if so == 'recommend':
        ...
        question_list = ShareBoard.objects.annotate(num_voter=Count('voter')).order_by('-num_voter', '-write_dttm')

    elif so == 'popular':

        question_list = ShareBoard.objects.annotate(num_answer=Count('answer')).order_by('-num_answer', '-write_dttm')
    ...
    py_boards = ShareBoard.objects.filter(
...
def board_write(request):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'shareboard/board_write.html', context)
...
def board_detail(request, pk):
    ...
    response = render(request, 'shareboard/board_detail.html', {
        'board': board,
...
def board_modify(request, pk):
    ...
    if request.method == 'GET':
        ...
        context['forms'] = write_form
        return render(request, 'shareboard/board_modify.html', context)
...
def like(request, pk):
    article = get_object_or_404(ShareBoard, pk=pk)

    if request.user in article.like_users.all():
        article.like_users.remove(request.user)
    else:
        article.like_users.add(request.user)
        return redirect('shareboard:board_detail', article.pk)
